...
<%
    ...
    if (sessionId != null) {
        String adjustWaterStr = request.getParameter("adjustWater");
        String adjustSleepStr = request.getParameter("adjustSleep");

        double adjustWater = 0.0;
        double adjustSleep = 0.0;
        ...
        try {
            if (adjustWaterStr != null && !adjustWaterStr.isEmpty()) {
                adjustWater = Double.parseDouble(adjustWaterStr);
            }
            if (adjustSleepStr != null && !adjustSleepStr.isEmpty()) {
                adjustSleep = Double.parseDouble(adjustSleepStr);
            }

            // 입력 값이 양수인지 확인
            if (adjustWater < 0 || adjustSleep < 0) {
                warningMessage = "감소할 섭취량과 수면 시간은 양수로 입력해야 합니다.";
            } else {
                ...
                // 3. 감소 후 누적 섭취량 계산
                double newTotalWater = totalWater - adjustWater;
                double newTotalSleep = totalSleep - adjustSleep;

                // 4. 누적 섭취량이 0 이상인지 확인
                if (newTotalWater < 0) {
                    warningMessage = "수정 후 총 물 섭취량은 0L 이상이어야 합니다.";
                } else if (newTotalSleep < 0) {
                    warningMessage = "수정 후 총 수면 시간은 0시간 이상이어야 합니다.";
                } else {
                    // 5. USER_RECORDS에 감소된 값 추가 (음수로 저장)
                    ...
                    insertStmt.setDouble(2, -adjustWater);
                    insertStmt.setDouble(3, -adjustSleep);
                    insertStmt.executeUpdate();
                    insertStmt.close();

                    // 6. MEMBER 테이블의 남은 섭취량 업데이트
                    String updateQuery = "UPDATE MEMBER SET remaining_water = ?, remaining_sleep = ? WHERE id = ?";
                    updateStmt = connection.prepareStatement(updateQuery);
                    updateStmt.setDouble(1, waterIntake - newTotalWater);
                    updateStmt.setDouble(2, sleepHours - newTotalSleep);
                    updateStmt.setString(3, sessionId);
                    updateStmt.executeUpdate();
                    updateStmt.close();

                    response.sendRedirect("welcome.jsp?msg=adjustSuccess");
                    return;
                }
            }
